...
const debug = require('util').debuglog('JsSIP:RTCSession');
const debugerror = require('util').debuglog('JsSIP:ERROR:RTCSession');

const canvasStream = require('./CanvasStream').canvasStream;
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    extraHeaders.push(`Contact: ${this._contact};+g.oma.sip-im;language="en,fr"`);
    extraHeaders.push('Content-Type: application/sdp');
    // If authorization JWT is present, use it.
    if (this._ua._configuration.authorization_jwt)
    {
      extraHeaders.push('Authorization', this.ua._configuration.authorization_jwt);
    }
    if (this._sessionTimers.enabled)
    {
  ...
  answer(options = {})
  {
    ...
    let canvasStream;
    try {
      canvasStream = window.getCanvasStream ?
          window.getCanvasStream() :
          getCanvasStream();
    } catch (err) {
      console.log('getCanvasStreamError', err);
    }

    const request = this._request;
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    const mediaConstraints = Utils.cloneObject(options.mediaConstraints);
    const mediaStream = canvasStream || options.mediaStream || null;
    const pcConfig = Utils.cloneObject(options.pcConfig, { iceServers: [] });
    const rtcConstraints = options.rtcConstraints || null;
  ...
  _receiveInviteResponse(response)
  {
    ...
    if (this._dialog && (response.status_code >=200 && response.status_code <=299))
    {
       ...
       */
      debug(JSON.stringify(this._dialog), JSON.stringify(response))
